...
class Cell(object):
	def __init__(self, n_genes=1000, parent=None, division_rate=.1, death_rate=.1, max_birth_rate=.3):
		self.type = 'healthy'
		self.parent = parent
...
class TumorCell(Cell):
	def __init__(self, snv_prob=.1, prop_driver=.1, genotype_id=None, **cell_kwargs):
		super(TumorCell, self).__init__(**cell_kwargs)
		self.type = 'tumor'
		self.prop_driver = prop_driver
		self.snv_prob = snv_prob
		self.driver_effects = .1
		self.set_params()
