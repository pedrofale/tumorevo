...
def plot_deme(n_cells, genotype_counts, populations_df, adjacency_df, color_by, average_radius=10, colormap="gnuplot", ax=None):
...
def plot_tree(genotype_parents, populations_df, adjacency_df, color_by, genotype_counts=None, labels=False, filter_clones=True, colormap="gnuplot", ax=None):
	# Re-use PyMuller to get the same genotype colors
	x = populations_df['Generation'].unique()
	y_table = pymuller.logic._get_y_values(populations_df, adjacency_df, 10)
	final_order = y_table.columns.values
			...
			if clone != root:
				if genotype_counts[clone] == 0:
					if clone in new_parent_dict.values():
						...
						children_pos = [i for i, x in enumerate(list(new_parent_dict.values())) if x == clone]
	...
	for node in G.nodes():
		...
		color_values.append(matplotlib.colors.to_hex(color_map[np.where(final_order == node)[0]][0]))
	
	pos = graphviz_layout(G, prog='dot')
	nx.draw(G, pos, with_labels=labels, arrows=False, node_color=color_values, labels=label_values, ax=ax)
	return ax
